...
module SEPA
  class DirectDebit < Message
    ...
    self.known_schemas = [ PAIN_008_001_02, PAIN_008_003_02, PAIN_008_002_02, PAIN_008_001_08 ]
          ...
          builder.CdtrAgt do
            builder.FinInstnId do
              if group[:account].bic
                (schema_name == PAIN_008_001_08) ? builder.BICFI(group[:account].bic) : builder.BIC(group[:account].bic)
              else
                builder.Othr do
    ...
    def build_transaction(builder, transaction, schema_name)
      builder.DrctDbtTxInf do
        ...
        builder.DbtrAgt do
          builder.FinInstnId do
            if transaction.bic
              (schema_name == PAIN_008_001_08) ? builder.BICFI(transaction.bic) : builder.BIC(transaction.bic)
            else
              builder.Othr do
                builder.Id('NOTPROVIDED')
              end
